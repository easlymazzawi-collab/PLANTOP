...
def leaderboard(limit: int = 10) -> list[dict]:
    ...
    se = plat.get("share_event") or {}
    if not se.get("enabled"):
        return []
    period_start = se.get("period_start")
    period_end = se.get("period_end")
    with connect() as conn:
        if period_start or period_end:
            clauses = ["u.ref_code IS NOT NULL", "u.ref_code != ''"]
            params: list = []
            join_extra = ""
            if period_start:
                join_extra += " AND sc.clicked_at >= ?"
                params.append(period_start)
            if period_end:
                join_extra += " AND sc.clicked_at <= ?"
                params.append(period_end)
            rows = conn.execute(
                f"""SELECT u.telegram_id, u.username, u.first_name, u.ref_code,
                           COUNT(sc.user_id) AS clicks
                    FROM users u
                    LEFT JOIN share_clicks sc ON sc.ref_code=u.ref_code{join_extra}
