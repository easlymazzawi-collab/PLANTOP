...
def should_protect_content(user_id: int) -> bool:
    """VIP + allow_forward → không protect; còn lại chặn forward/save."""
    if not user_is_vip(user_id):
        return True
    init_db()
    with connect() as conn:
        row = conn.execute(
            "SELECT allow_forward FROM users WHERE telegram_id=?",
            (user_id,),
        ).fetchone()
    if row and row["allow_forward"]:
        return False
    return True
