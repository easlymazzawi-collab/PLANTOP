...
from research_platform.vip import can_access_archive, get_user_vip_status, list_vip_plans, should_protect_content, user_is_vip
...
async def _copy_item(bot, chat_id: int, item: dict, *, protect_content: bool = True) -> bool:
    ...
    for mid in msg_ids:
        try:
            await bot.copy_message(
                chat_id=chat_id, from_chat_id=src_chat, message_id=mid,
                protect_content=protect_content,
            )
            sent += 1
...
async def deliver_day_to_user(bot, user_id: int, day: dict) -> tuple[int, int]:
    ...
    sent = 0
    total = len(items)
    protect = should_protect_content(user_id)
    for item in items:
        if item["seq"] <= start_seq:
            continue
        if await _copy_item(bot, user_id, item, protect_content=protect):
            sent += 1
...
def build_handlers_for_bot(bot_cfg: dict, bot_db_id: int | None):
    ...
    async def _handle_start_payload(msg: Message, payload: str) -> None:
    ...
    async def _replay_pending(msg: Message, pending: str) -> bool:
        if not pending or pending == "menu":
            return False
        if pending == "today":
            await _handle_day(msg, topic_label_vn(today_vn()))
            return True
        if pending.startswith("day:"):
            await _handle_day(msg, pending[4:])
            return True
        if pending.startswith("start:"):
            await _handle_start_payload(msg, pending[6:])
            return True
        if pending == "start":
            await _handle_start_payload(msg, "")
            return True
        return False
    ...
    async def cmd_start(msg: Message):
        ...
        pending_key = f"start:{payload}" if payload else "start"
        if not await gate_or_prompt(msg.bot, msg.from_user.id, pending=pending_key):
            return
        await _handle_start_payload(msg, payload)
    ...
    async def cb_mem_recheck(cb: CallbackQuery):
        ...
        if not ok:
            ...
            return
        await cb.answer()
        await cb.message.answer("✅ Đã join — tiếp tục.", reply_markup=_keyboard())
        pending = pop_pending(cb.from_user.id)
        if pending:
            await _replay_pending(cb.message, pending)

    @dp.message(Command("today"))
    async def cmd_today(msg: Message):
        if not await gate_or_prompt(msg.bot, msg.from_user.id, pending="today"):
            return
        await _handle_day(msg, topic_label_vn(today_vn()))
    ...
    async def cmd_day(msg: Message):
        ...
        if len(parts) < 2:
            ...
            return
        label = parts[1].strip()
        if not await gate_or_prompt(msg.bot, msg.from_user.id, pending=f"day:{label}"):
            return
