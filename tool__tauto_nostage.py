...
async def _run_all_topics_only(*, force_run=False):
    ...
    from research_platform.run_queue import run_platform_queue

    plat = load_platform_config()
    if plat.get("enabled") and plat.get("orchestrator_running", True) and list_bots_config(plat):
        async def _run_topic(src_chat_id, topic_id, branch, title):
            await _try_auto_source_topic(
                src_chat_id, topic_id, title,
                manual=True, force_run=force_run, branch=branch,
            )
            return True

        results = await run_platform_queue(_run_topic, notify=_notify)
        return len(results)
